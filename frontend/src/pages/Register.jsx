...
export default function Register() {
  ...
  const handleSubmit = async e => {
    ...
    setLoading(true);
    try {
      const base = import.meta.env.VITE_API_URL || 'https://atomquest-gzbs.vercel.app/api';
      ...
      navigate('/employee');
    } catch { setError('Network error — is the backend reachable?'); }
    ...
    <div style={{ minHeight:'100vh', display:'flex', background:'#F8F8F6' }}>
      ...
      <div style={{ flex:1, display:'flex', flexDirection:'column', justifyContent:'center', alignItems:'center', padding:48, background:'linear-gradient(145deg,#083D2E,#1D9E75 60%,#2EC68E)' }}>
        <div style={{ maxWidth:380, color:'#fff' }}>
          ...
          <h1 style={{ fontSize:30, fontWeight:800, marginBottom:12, lineHeight:1.2 }}>Join as<br/>Employee</h1>
          <p style={{ fontSize:14, opacity:.82, lineHeight:1.8, marginBottom:28 }}>
            Create your employee account to start setting SMART annual goals with AI assistance.
          </p>
          {['Set up to 8 SMART goals per year','AI suggests goals based on your role','Track Q1–Q4 quarterly progress','Manager check-ins and feedback loop'].map(t => (
          ...
          <div style={{ marginTop:28, padding:'12px 16px', background:'rgba(255,255,255,0.1)', borderRadius:10, fontSize:12, border:'0.5px solid rgba(255,255,255,0.2)', lineHeight:1.7 }}>
            <strong>Manager or Admin?</strong><br/>
            Accounts for managers and admins are created by your organisation's Admin. Contact your Admin to get access.
          </div>
        </div>
      </div>
      ...
      <div style={{ width:480, display:'flex', flexDirection:'column', justifyContent:'center', padding:'48px 44px', background:'#fff', overflowY:'auto' }}>
        <div style={{ maxWidth:380, width:'100%', margin:'0 auto' }}>
          ...
          <h2 style={{ fontSize:22, fontWeight:800, color:'#1A1A18', marginBottom:3, letterSpacing:'-0.4px' }}>Create employee account</h2>
          ...
          <div style={{ marginBottom:20, padding:'10px 14px', background:'#E1F5EE', borderRadius:8, border:'0.5px solid #9FE1CB', fontSize:12, color:'#0F6E56', lineHeight:1.6 }}>
            <strong>Employee registration only.</strong> Manager and Admin accounts are created by your organisation's Admin.
          </div>
          ...
          <form onSubmit={handleSubmit}>
            ...
            {error && (
              <div style={{ marginBottom:14, padding:'9px 12px', background:'#FCEBEB', border:'0.5px solid #E24B4A', borderRadius:6, fontSize:12, color:'#791F1F' }}>
                {error}
              </div>
            )}
          ...
          <p style={{ textAlign:'center', fontSize:10, color:'#C0BEB5', marginTop:20, lineHeight:1.6 }}>
